...
用户 2026-09-19 拍板：公开仓库统一英文。改的是 11 条工具描述、27 条参数说明、
所有运行时报错，以及 BoardKit 剩的 4 行（含两条会经 HTTP 回给 agent 的）。

**测量教训**：按「非 ASCII」数会把破折号算成中文，第一次量出来的范围比实际大五倍。
要按 CJK 范围（`grep -c '[一-龥]'`）数。

**测试里的中文分两类**：夹具数据不动；`MemoryTests.swift:272` 的「门禁」和
`APITests.swift:826` 的百分号编码用例是**故意用非 ASCII 测非 ASCII 行为**的，
翻了等于废掉测试。只改了 8 处断言，每处都断言在被翻掉的那个字符串上。

顺带补了 `plan_memory_add` 描述里缺的 100 条上限——`memory.md` 基于它写了整套
处置流程，而工具描述一个字没提，agent 撞上拒绝时最该忍住的动作（删别的条目腾位置）
恰好是最顺手的。
